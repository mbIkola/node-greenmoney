...
class GreenMoney {
  ...
  constructor(clientId, apiPassword) {
  ...
  oneTimeDraftRTV( name, phone, address1, city, state, zip, routingNumber, accountNumber, bankName, checkAmount, additionalOpts ) {
    let opts = {name, phone, address1, city, state, zip, routingNumber, accountNumber, bankName};
  ...
  apiCall(method, params) {
      ...
      return request.post( {url: this.apiUrl,  formData} )
